from __future__ import annotations

from sqlite3 import Row
from typing import Any


def project_from_row(row: Row) -> dict[str, Any]:
    return {
        "id": row["id"],
        "name": row["name"],
        "description": row["description"],
        "source_image_path": row["source_image_path"],
        "panorama_image_path": row["panorama_image_path"],
        "created_at": row["created_at"],
        "updated_at": row["updated_at"],
    }


def character_asset_from_row(row: Row) -> dict[str, Any]:
    return {
        "id": row["id"],
        "project_id": row["project_id"],
        "name": row["name"],
        "model_path": row["model_path"],
        "created_at": row["created_at"],
        "updated_at": row["updated_at"],
    }


def character_instance_from_row(row: Row) -> dict[str, Any]:
    return {
        "id": row["id"],
        "project_id": row["project_id"],
        "scene_state_id": row["scene_state_id"],
        "character_asset_id": row["character_asset_id"],
        "name": row["name"],
        "position_x": row["position_x"],
        "position_y": row["position_y"],
        "position_z": row["position_z"],
        "rotation_x": row["rotation_x"],
        "rotation_y": row["rotation_y"],
        "rotation_z": row["rotation_z"],
        "scale": row["scale"],
        "visible": bool(row["visible"]),
        "created_at": row["created_at"],
        "updated_at": row["updated_at"],
    }


def scene_state_from_row(row: Row) -> dict[str, Any]:
    return {
        "id": row["id"],
        "project_id": row["project_id"],
        "name": row["name"],
        "description": row["description"],
        "sort_order": row["sort_order"],
        "shot_number": row["shot_number"],
        "shot_size": row["shot_size"],
        "camera_move": row["camera_move"],
        "action_notes": row["action_notes"],
        "prompt_notes": row["prompt_notes"],
        "camera_position_x": row["camera_position_x"],
        "camera_position_y": row["camera_position_y"],
        "camera_position_z": row["camera_position_z"],
        "camera_target_x": row["camera_target_x"],
        "camera_target_y": row["camera_target_y"],
        "camera_target_z": row["camera_target_z"],
        "camera_fov": row["camera_fov"],
        "created_at": row["created_at"],
        "updated_at": row["updated_at"],
    }


def environment_variant_from_row(row: Row) -> dict[str, Any]:
    return {
        "id": row["id"],
        "project_id": row["project_id"],
        "name": row["name"],
        "source_image_path": row["source_image_path"],
        "panorama_image_path": row["panorama_image_path"],
        "status": row["status"],
        "generator": row["generator"],
        "source_prompt": row["source_prompt"],
        "panorama_prompt": row["panorama_prompt"],
        "negative_prompt": row["negative_prompt"],
        "notes": row["notes"],
        "width": row["width"],
        "height": row["height"],
        "horizon_y": row["horizon_y"],
        "floor_y": row["floor_y"],
        "floor_grid_size": row["floor_grid_size"],
        "floor_grid_divisions": row["floor_grid_divisions"],
        "placement_radius": row["placement_radius"],
        "default_character_scale": row["default_character_scale"],
        "camera_height": row["camera_height"],
        "calibration_notes": row["calibration_notes"],
        "is_active": bool(row["is_active"]),
        "created_at": row["created_at"],
        "updated_at": row["updated_at"],
    }
